...
CFEM_DG_EulerSolver::CFEM_DG_EulerSolver(CGeometry *geometry, CConfig *config, unsigned short iMesh) : CSolver() {
  ...
  if( config->GetViscous() ) {
    ...
    unsigned int sizeSur = 3*nVar*max(nIntegrationMax,nDOFsMax);
...
void CFEM_DG_EulerSolver::ResidualFaces(CConfig             *config,
  ...
  /* Initilize the variable for the timing routines. */
  double tick = 0.0;
  ...
  for(unsigned long l=indFaceBeg; l<indFaceEnd; l+=3) {

    /* Set lBeg and lEnd for this chunk of elements. */
    const unsigned long lBeg   = l;
    const unsigned long lEnd   = min(l+3, indFaceEnd);
    ...
    /* Get the necessary data from the standard element. */
    ...
    /* Compute the inviscid fluxes in the integration points of the
       chunk of faces. */
    InviscidFluxesInternalMatchingFace(config, lBeg, lEnd, NPad,
...
void CFEM_DG_EulerSolver::InviscidFluxesInternalMatchingFace(
  ...
  /* Get the required information from the corresponding standard face,
     which is the same for the chunk of elements considered. */
  ...
  /* Define the array of pointers to be used later on in the call to
     ComputeInviscidFluxesFace. */
  const su2double* arrNorm[8];
  ...
  for(unsigned long l=lBeg; l<lEnd; ++l) {
    ...
    /* Store the pointer to the normal information of this face. This is used
       later on in the call to ComputeInviscidFluxesFace. */
    ...
    for(unsigned short i=0; i<nDOFsFace0; ++i) {
      const su2double *solDOF = VecWorkSolDOFs[timeLevelFace].data()
                              + nVar*(DOFs[i] - offset);
      su2double       *sol    = solFace + NPad*i + nVar*ll;
  ...
  config->GEMM_Tock(tick, "InviscidFluxesInternalMatchingFace2", nInt, NPad, nDOFsFace0);
  ...
  const unsigned short llEnd = lEnd - lBeg;
  ComputeInviscidFluxesFace(config, llEnd, NPad, nInt, arrNorm,
                            solIntL, solIntR, fluxes, numerics);
}
  ...
  for(unsigned short l=0; l<nFacesFused; ++l) {
    ...
    for(unsigned short i=0; i<nInt; ++i) {
      ...
      /* Easier storage of the left and right solution and the normals
         for this integration point. */
      const su2double *UL      = solIntL + i*NPad + l*nVar;
            su2double *UR      = solIntR + i*NPad + l*nVar;
...
void CFEM_DG_EulerSolver::BoundaryStates_Inlet(CConfig                  *config,
  ...
  /*--- Apply the subsonic inlet boundary conditions to compute the right
        state in the integration points. First retrieve the specified total
        conditions for this inlet. ---*/
...
void CFEM_DG_EulerSolver::BoundaryStates_Outlet(CConfig                  *config,
  ...
  /*--- Apply the subsonic inlet boundary conditions to compute the right
        state in the integration points. First retrieve the specified back
        pressure for this outlet.  Nondimensionalize, if necessary. ---*/
  string Marker_Tag = config->GetMarker_All_TagBound(val_marker);
...
void CFEM_DG_EulerSolver::BoundaryStates_Riemann(CConfig                  *config,
  ...
  switch( config->GetKind_Data_Riemann(Marker_Tag) ) {
    ...
    case STATIC_SUPERSONIC_INFLOW_PT: {
      ...
      /* Loop over the number of fused faces and the integration points
         and set the right state in the integration points. */
      const unsigned long nBytes = nVar*sizeof(su2double);
      for(unsigned short l=0; l<nFacesFused; ++l) {
        for(unsigned short i=0; i<nInt; ++i) {
          su2double *UR = solIntR + i*NPad + l*nVar;
  ...
  switch( nDim ) {
    case 2: {
      ...
      /* Two-dimensional simulation. Loop over the number of fused
         faces and integration points. */
      for(unsigned short l=0; l<nFacesFused; ++l) {
        for(unsigned short i=0; i<nInt; ++i) {
    ...
    case 3: {
      ...
      /* Three-dimensional simulation. Loop over the number of fused
         faces and integration points. */
      for(unsigned short l=0; l<nFacesFused; ++l) {
        for(unsigned short i=0; i<nInt; ++i) {
...
void CFEM_DG_EulerSolver::BC_Euler_Wall(CConfig                  *config,
  ...
  for(unsigned long l=surfElemBeg; l<surfElemEnd; l+=3) {

    /* Set lBeg and lEnd for this chunk of faces. */
    const unsigned long lBeg   = l;
    const unsigned long lEnd   = min(l+3, surfElemEnd);
    const unsigned short llEnd = lEnd - lBeg;
...
void CFEM_DG_EulerSolver::BC_Far_Field(CConfig                  *config,
  ...
  for(unsigned long l=surfElemBeg; l<surfElemEnd; l+=3) {
    ...
    for(unsigned short ll=0; ll<llEnd; ++ll) {
      for(unsigned short i=0; i<nInt; ++i) {
        su2double *UR = solIntR + i*NPad + ll*nVar;
        for(unsigned short j=0; j<nVar; ++j)
  ...
  for(unsigned long l=surfElemBeg; l<surfElemEnd; l+=3) {
    ...
    switch( nDim ) {
      ...
      case 2: {
        ...
        /* 2D simulation. Loop over the fused faces and the integration
           points to apply the symmetry condition. */
        for(unsigned short ll=0; ll<llEnd; ++ll) {
          for(unsigned short i=0; i<nInt; ++i) {

            /* Easier storage of the left and right solution and the normals
               for this integration point. */
            const su2double *UL      = solIntL + NPad*i + nVar*ll;
      ...
      case 3: {
        ...
        /* 3D simulation. Loop over the fused faces and the integration
           points to apply the symmetry condition. */
        for(unsigned short ll=0; ll<llEnd; ++ll) {
          for(unsigned short i=0; i<nInt; ++i) {
...
void CFEM_DG_EulerSolver::ResidualInviscidBoundaryFace(
  ...
  /* Store the normals of the faces in an array of pointers to be used
     later on in the call to ComputeInviscidFluxesFace. */
  const su2double* arrNorm[8];
  for(unsigned short l=0; l<nFacesFused; ++l)
  ...
  ComputeInviscidFluxesFace(config, nFacesFused, NPad, nInt, arrNorm,
  ...
  /*------------------------------------------------------------------------*/
  /*--- Step 2: Compute the contribution to the residuals from the       ---*/
  /*---         integration over the surface elements.                   ---*/
  /*------------------------------------------------------------------------*/
  ...
  /* Loop over the number of fused faces to store the residual in
  ...
  for(unsigned short l=0; l<nFacesFused; ++l) {
    ...
    for(unsigned short i=0; i<nDOFs; ++i)
      memcpy(resFace+nVar*i, solInt0+NPad*i+nVar*l, nBytes);
  }
}
...
void CFEM_DG_EulerSolver::LeftStatesIntegrationPointsBoundaryFace(CConfig                  *config,
  ...
  for(unsigned short l=0; l<nFacesFused; ++l) {
    ...
    for(unsigned short i=0; i<nDOFs; ++i) {
      ...
      su2double       *sol    = solFace + NPad*i + l*nVar;
      memcpy(sol, solDOF, nBytes);
    }
  }
...
void CFEM_DG_EulerSolver::ComputeInviscidFluxesFace(CConfig              *config,
  ...
  switch( config->GetRiemann_Solver_FEM() ) {
    ...
    case ROE: {
      ...
      switch( nDim ) {
        ...
        case 2: {
          ...
          /* Two dimensional simulation. Loop over the number of faces fused. */
          for(unsigned short l=0; l<nFacesFused; ++l) {
            ...
            for(unsigned long i=0; i<nPoints; ++i) {
              ...
              /* Easier storage of the left and right solution, the face normals
                 and the flux vector for this point. */
              const unsigned long offPointer = i*NPad + l*nVar;

              const su2double *UL   = solL + offPointer;
              const su2double *UR   = solR + offPointer;
              const su2double *norm = normals + 3*i;   // 3 = nDim+1
                    su2double *flux = fluxes + offPointer;

              const su2double nx = norm[0], ny = norm[1], area = norm[2];

              /*--- Compute the primitive variables of the
              ...
              /*--- Compute the absolute values of the three eigenvalues, apply
                    the entropy correction and multiply by the area to obtain
              ...
              /*--- Compute the Roe flux vector, which is
                    0.5*(FL + FR - |A|(UR-UL)). ---*/
        ...
        case 3: {
          ...
          /* Three dimensional simulation. Loop over the number of faces fused. */
          for(unsigned short l=0; l<nFacesFused; ++l) {
            ...
            for(unsigned long i=0; i<nPoints; ++i) {
              ...
              const su2double *UL   = solL + offPointer;
              const su2double *UR   = solR + offPointer;
              const su2double *norm = normals + 4*i;   // 4 = nDim+1
                    su2double *flux = fluxes + offPointer;
              ...
              /*--- Compute the difference of the conservative
                    mean flow variables. ---*/
              const su2double dr  = UR[0] - UL[0];
              const su2double dru = UR[1] - UL[1];
              const su2double drv = UR[2] - UL[2];
              ...
              const su2double abv4 = gm1*(alphaAvg*dr - uAvg*dru - vAvg*drv
                                   -      wAvg*drw + drE);
    ...
    default: {
      ...
      /* Loop over the number of faces fused. */
      for(unsigned short l=0; l<nFacesFused; ++l) {
...
void CFEM_DG_NSSolver::ResidualFaces(CConfig             *config,
  ...
  /* Initilize the counters for the timing routines. */
  double tick = 0.0;
  double tick2 = 0.0;
  ...
  for(unsigned long l=indFaceBeg; l<indFaceEnd; l+=3) {
    ...
    /*--- Get the information from the standard face, which is the same
          for all faces in the chunk considered. ---*/
    const unsigned short ind  = matchingInternalFaces[l].indStandardElement;
    const unsigned short nInt = standardMatchingFacesSol[ind].GetNIntegration();
    ...
    config->Tick(&tick);
    /* Compute the inviscid fluxes in the integration points of the chunk
       of faces. */
    InviscidFluxesInternalMatchingFace(config, lBeg, lEnd, NPad,
                                       solIntL, solIntR, fluxes, numerics);
    ...
    for(unsigned long ll=lBeg; ll<lEnd; ++ll) {
      const unsigned short llRel = ll - lBeg;
      ...
      for(unsigned short i=0; i<nDOFsElem0; ++i) {
        ...
        su2double       *sol    = solElem + NPad*i + nVar*llRel;
        memcpy(sol, solDOF, nBytes);
      }
    }
    ...
    /*---         integration over the internally matching face.           ---*/
    /*------------------------------------------------------------------------*/

    config->Tick(&tick);

    /* Determine the number of DOFs on both sides of the face. */
    const unsigned short nDOFsFace0 = standardMatchingFacesSol[ind].GetNDOFsFaceSide0();
    const unsigned short nDOFsFace1 = standardMatchingFacesSol[ind].GetNDOFsFaceSide1();
    ...
    if( symmetrizingTermsPresent ) {
      ...
      for(unsigned short ll=0; ll<llEnd; ++ll) {
        ...
        for(unsigned short i=0; i<nDOFsElem0; ++i)
          memcpy(resElem0+nVar*i, solIntL+NPad*i+nVar*ll, nBytes);

        /* Loop over the DOFs of the element and copy the data for side 1. */
        for(unsigned short i=0; i<nDOFsElem1; ++i)
          memcpy(resElem1+nVar*i, solIntR+NPad*i+nVar*ll, nBytes);
      }
  ...
  switch( nDim ) {
    ...
    case 2: {
      ...
      for(unsigned short i=0; i<nInt; ++i) {
        ...
        const su2double *sol0   = solInt0 + offPointer;
        const su2double *sol1   = solInt1 + offPointer;
        const su2double *normal = metricNormalsFace + 3*i; // 3 = nDim+1.

        su2double *flux = symmFluxes + i*nDim*NPad + indFaceChunk*nVar;
    ...
    case 3: {
      ...
      for(unsigned short i=0; i<nInt; ++i) {
        ...
        const su2double *sol0   = solInt0 + offPointer;
        const su2double *sol1   = solInt1 + offPointer;
        const su2double *normal = metricNormalsFace + 4*i;  // 4 = nDim+1.
  ...
  for(unsigned long l=surfElemBeg; l<surfElemEnd; l+=3) {
    ...
    for(unsigned long ll=lBeg; ll<lEnd; ++ll) {
      ...
      for(unsigned short i=0; i<nDOFsElem; ++i) {
        ...
        memcpy(solElem+NPad*i+nVar*llRel, solDOF, nBytes);
      }
    }
      ...
      switch( nDim ) {
        ...
        case 2: {
          ...
          for(unsigned short i=0; i<nInt; ++i) {
            ...
            /* Easier storage of the left and right solution and the normals
               for this integration point. */
            const unsigned short pointerOffset = NPad*i + nVar*llRel;
...
void CFEM_DG_NSSolver::ViscousBoundaryFacesBCTreatment(
  ...
  su2double *kOverCvInt   = viscosityInt + nFacesFused*nInt;
  su2double *gradSolInt   = kOverCvInt   + nFacesFused*nInt;
  su2double *fluxes       = gradSolInt   + NPad*nInt*nDim;
  su2double *viscFluxes   = fluxes       + NPad*max(nInt*nDim, (int) nDOFsElem);
  ...
  /* Loop over the fused faces to set the solution of the elements. */
  for(unsigned short l=0; l<nFacesFused; ++l) {
    ...
    for(unsigned short i=0; i<nDOFsElem; ++i) {
      const su2double *solDOF = VecWorkSolDOFs[timeLevel].data()
                              + nVar*(DOFsElem[i] - offset);
      su2double       *sol    = solElem + NPad*i + nVar*l;
      memcpy(sol, solDOF, nBytes);
    }
  }
  ...
  ResidualViscousBoundaryFace(config, conv_numerics, nFacesFused, NPad, surfElem,
	                            solIntL, solIntR, gradSolInt, fluxes, viscFluxes,
                              viscosityInt, kOverCvInt, resFaces, indResFaces);
}
...
void CFEM_DG_NSSolver::ResidualViscousBoundaryFace(
  ...
  for(unsigned short l=0; l<nFacesFused; ++l) {
    ...
    /* Loop over the DOFs and copy the data. */
    for(unsigned short i=0; i<nDOFs; ++i)
      memcpy(resFace+nVar*i, viscFluxes+NPad*i+nVar*l, nBytes);
  }
  ...
  if( symmetrizingTermsPresent ) {
    ...
    for(unsigned short l=0; l<nFacesFused; ++l) {
      ...
      for(unsigned short i=0; i<nDOFsElem; ++i)
        memcpy(resElem+nVar*i, fluxes+NPad*i+nVar*l, nBytes);
    }
  }
}
